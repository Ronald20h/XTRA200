...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription('حماية السيرفر من إنشاء ويب هوكس')
        .addStringOption(o =>
    ...
    if (!_isProtectionAuthorized(interaction)) {
      return interaction.reply({ content: '❌ هذا الأمر لصاحب البوت أو صاحب السيرفر فقط!', ephemeral: true });
        ...
        protectDB.set(`anti_webhook_${interaction.guild.id}`, status === 'on');
        const embed = new EmbedBuilder()
            .setColor(status === 'on' ? '#00FF00' : '#FF0000')
            .setTitle(status === 'on' ? '✅ تم تفعيل حماية الويب هوك' : '❌ تم تعطيل حماية الويب هوك')
            .addFields({ name: '🔨 العقوبة', value: 'باند فوري + حذف الويب هوك تلقائياً', inline: false })
