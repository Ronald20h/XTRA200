...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('anti-role-create')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
        .setDescription('حماية السيرفر من إنشاء رتب جديدة')
        .addStringOption(o =>
    ...
    if (!_isProtectionAuthorized(interaction)) {
      return interaction.reply({ content: '❌ هذا الأمر لصاحب البوت أو صاحب السيرفر فقط!', ephemeral: true });
        ...
        protectDB.set(`anti_role_create_${interaction.guild.id}`, status === 'on');
        const embed = new EmbedBuilder()
            .setColor(status === 'on' ? '#00FF00' : '#FF0000')
            .setTitle(status === 'on' ? '✅ تم تفعيل حماية إنشاء الرتب' : '❌ تم تعطيل حماية إنشاء الرتب')
            .addFields({ name: '🔨 العقوبة', value: 'باند فوري + حذف الرتبة تلقائياً', inline: false })
