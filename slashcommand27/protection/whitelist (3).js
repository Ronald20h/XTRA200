...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub =>
      sub.setName('add-user')
        .setDescription('✅ إضافة عضو أو بوت (منشن أو ID)')
        .addUserOption(opt => opt.setName('user').setDescription('منشن العضو أو البوت').setRequired(false))
        .addStringOption(opt => opt.setName('id').setDescription('أو اكتب الـ ID يدوياً').setRequired(false))
  ...
  async execute(interaction) {
    ...
    if (sub === 'add-user') {
      ...
      if (!mentionedUser && !idInput)
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('#ED4245').setDescription('❌ لازم تحدد عضو بالمنشن أو الـ ID!')], ephemeral: true });

      let targetId, name, isBot = false;
      if (mentionedUser) {
        targetId = mentionedUser.id;
        name     = mentionedUser.username;
        isBot    = mentionedUser.bot;
      } else {
        targetId = idInput.trim().replace(/[<@!>]/g, '');
        if (!/^\d{17,20}$/.test(targetId))
          return interaction.reply({ embeds: [new EmbedBuilder().setColor('#ED4245').setDescription('❌ الـ ID غير صحيح!')], ephemeral: true });
        try { const u = await interaction.client.users.fetch(targetId); name = u.username; isBot = u.bot; } catch { name = targetId; }
      }
      const wl = getWL();
    ...
    // ─── remove-user ───
    if (sub === 'remove-user') {
      const mentionedUser2 = interaction.options.getUser('user');
      const idInput2       = interaction.options.getString('id');
      const type           = interaction.options.getString('type');

      if (!mentionedUser2 && !idInput2)
        return interaction.reply({ embeds: [new EmbedBuilder().setColor('#ED4245').setDescription('❌ لازم تحدد عضو بالمنشن أو الـ ID!')], ephemeral: true });

      let targetId, name;
      if (mentionedUser2) {
        targetId = mentionedUser2.id;
        name     = mentionedUser2.username;
      } else {
        targetId = idInput2.trim().replace(/[<@!>]/g, '');
        try { const u = await interaction.client.users.fetch(targetId); name = u.username; } catch { name = targetId; }
      }
      const wl = getWL();
