const { SlashCommandBuilder, ChatInputCommandInteraction, Client, EmbedBuilder, PermissionsBitField,PermissionFlagsBits, } = require("discord.js");
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // الرسائل
        let messagedelete = await logsDB.get(`log_messagedelete_${interaction.guild.id}`);
        let messageupdate = await logsDB.get(`log_messageupdate_${interaction.guild.id}`);
        // الرتب
        let rolecreate = await logsDB.get(`log_rolecreate_${interaction.guild.id}`);
        let roledelete = await logsDB.get(`log_roledelete_${interaction.guild.id}`);
        let rolegive = await logsDB.get(`log_rolegive_${interaction.guild.id}`);
        let roleremove = await logsDB.get(`log_roleremove_${interaction.guild.id}`);
        // الرومات
        let channelcreate = await logsDB.get(`log_channelcreate_${interaction.guild.id}`);
        let channeldelete = await logsDB.get(`log_channeldelete_${interaction.guild.id}`);
        // البوتات
        let botadd = await logsDB.get(`log_botadd_${interaction.guild.id}`);
        // الباند و الطرد كيك
        let banadd = await logsDB.get(`log_banadd_${interaction.guild.id}`);
        let bandelete = await logsDB.get(`log_bandelete_${interaction.guild.id}`);
        let kickadd = await logsDB.get(`log_kickadd_${interaction.guild.id}`);
